    ...
    <el-container>
        <el-aside width="100%">
            ...
            <div v-if="list.length>0">
                ...
                <el-button type="primary" @click.stop="getPdf()" v-if="showPdf">导出图片</el-button>
            </div>
            ...
            <el-table :data="list" border style="width: 100%">
                ...
                <el-table-column label="生成图片">
                    <template slot-scope="scope">
                        <el-image style="width: auto; height: 120px;" :src="scope.row.synthesis" cover :preview-src-list="imgList"></el-image>
                    </template>
                </el-table-column>
            </el-table>
        </el-aside>
        ...
        <el-main style="opacity:0;">
            <div class="h5-box" ref="content" v-if="list.length>0 && lSwitch">
                <div class="list" :id="'menuItem1'">
                    <img class="bg" src="@/assets/mobile/bg.png">
                    <div class="content">
                        <div class="t-box" >
                            <div class="t" v-if="list[listIndex].productName">{{list[listIndex].productName | lineBreak | maxLength(40)}}</div>
                        </div>
                        <div class="c-box" >
                            ...
                            <div class="txt-box">
                                <div class="con">
                                    <div class="t-list">
                                        ...
                                        <div class="right" v-if="list[listIndex].productName">{{list[listIndex].productName | lineBreak | maxLength(55)}}</div>
                                    </div>
                                    <div class="t-list jj">
            ...
            <div class="pc-box" ref="content" v-if="list.length> 0 && !lSwitch">
                <div class="list" :id="'menuItem1'">
                    <img class="bg" src="@/assets/pc/bg.jpg">
                    <div class="content">
                        <div class="t-box" >
                            <div class="t" v-if="list[listIndex].productName">{{list[listIndex].productName | lineBreak | maxLength(45)}}</div>
                        </div>
                        <div class="c-box" >
                            <img class="c" :src="list[listIndex].picPath" v-if="list[listIndex].picPath">
                        </div>
                        <div class="bg4-box">
                            <div class="right"><div v-if="list[listIndex].productName">{{list[listIndex].productName | lineBreak | maxLength(150)}}</div></div>
                            <div class="right"><div v-if="list[listIndex].intro">{{list[listIndex].intro | lineBreak | maxLength(150)}}</div></div>
                            <div class="right"><div v-if="list[listIndex].software">{{list[listIndex].software | lineBreak | maxLength(150)}}</div></div>
                            <div class="right"><div v-if="list[listIndex].content">{{list[listIndex].content | lineBreak | maxLength(150)}}</div></div>
                            <div class="right"><div v-if="list[listIndex].hardware">{{list[listIndex].hardware | lineBreak | maxLength(150)}}</div></div>
                            <div class="right"><div v-if="list[listIndex].price">{{list[listIndex].price |  maxLength(150)}}</div></div>
                        </div>
                    </div>
                </div>
            </div>
...
import { nullToStr, imgToBase64, baseUrltoBlob } from '@/utils/tool'
export default {
    ...
    methods: {
        ...
        addimg(){
            ...
            if (this.lSwitch) {
                imgToBase64(require('@/assets/mobile/home.png')).then(res => { this.imgList.unshift(res) })
            } else {
                imgToBase64(require('@/assets/pc/home.jpg')).then(res => { this.imgList.unshift(res) })
            }
            this.htmlToCanvas();
        },
        // 异步html2canvas回调事件
        async htmlToCanvas() {
            ...
            that.toImg().then(res => {
                ...
                that.list[this.listIndex].synthesis = res;
        ...
        // 导出pdf
        async getPdf() {
            let loading = this.$loading({ lock: true, text: '等待下载队列', spinner: 'el-icon-loading', background: 'rgba(0, 0, 0, 0.7)'})
            const zip = new JSZip()
            const that = this
            const promises = [];
            
            for (let imgIndex = 0; imgIndex < this.imgList.length; imgIndex++) {
                let blob = baseUrltoBlob(this.imgList[imgIndex])
                zip.file((imgIndex+1) +'.jpg', blob, { binary: true })
                promises.push(blob);
            }
            Promise.all(promises).then(() => {
                zip.generateAsync({ type: 'blob' }).then((content) => {
                    ...
                    that.$message.success('图片下载完成')
                })
            }).catch((res) => { that.$message.warning('文件下载失败!') })
        },
    },
};
</script>
